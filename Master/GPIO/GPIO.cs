...
namespace RaspberryPi.GPIO
{
    ...
    public abstract class GPIO : IDisposable
    {
        ...
        protected GPIO(GPIOPins pin, GPIOPwmDivisor PWMDivisor, GPIOPwmChannel PWMChannel, GPIOPwmMarkspace PWMMarkspace, int PWMRange)
        {
            if (pin == GPIOPins.GPIO_NONE) throw new ArgumentException("Invalid pin");
            lock (_exportedPins)
            {
                ...
                try
                {
                    ...
                    this.PWMDivisor = PWMDivisor;
                    this.PWMChannel = PWMChannel;
                    this.PWMMarkspace = PWMMarkspace;
                    this.PWMRange = PWMRange;
                    this.PWMEnabled = GPIOPwmEnabled.False;
                }
                catch
                {
                    Dispose();
        ...
        GPIOPwmDivisor PWMDivisor;
        GPIOPwmChannel PWMChannel;
        GPIOPwmMarkspace PWMMarkspace;
        GPIOPwmEnabled PWMEnabled;
        int PWMRange;
        ...
        virtual public void SetPWMClock(uint PWMDivisor) { }
        virtual public void SetPWMMode(int markspace, int enabled) { }
        virtual public void SetPWMRange(int range) { }
        virtual public void SetPWMData(uint PWMData) { }
        ...
        public void PwmEnable(bool state)
        {
            if (state)
            {
                SetPWMMode((int)this.PWMMarkspace, 1);
                this.PWMEnabled = GPIOPwmEnabled.True;
            }
            else
            {
                SetPWMMode((int)this.PWMMarkspace, 0);
                this.PWMEnabled = GPIOPwmEnabled.False;
            }
        }
        ...
        public virtual void Dispose()
        {
            ...
            _disposed = true;
            if (PWMEnabled == GPIOPwmEnabled.True)
            {
                PwmEnable(false);
                PWMEnabled = GPIOPwmEnabled.False;
            }
