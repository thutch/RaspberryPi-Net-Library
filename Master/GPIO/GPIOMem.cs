...
namespace RaspberryPi.GPIO
{
    ...
    public class GPIOMem : GPIO
    {
        ...
        static GPIOMem()
        {
            ...
            if (!bcm2835_init())
                new Exception("Unable to initialize bcm2835.so library");
        }
        #endregion
        ...
        public GPIOMem(GPIOPins pin, GPIOPwmDivisor PWMDivisor, GPIOPwmChannel PWMChannel, GPIOPwmMarkspace PWMMarkspace, int PWMRange)
            : base(pin, PWMDivisor, PWMChannel, PWMMarkspace, PWMRange)
        {
            bcm2835_gpio_fsel(pin, (int)GPIODirection.Pwm);
            bcm2835_pwm_set_clock((uint)PWMDivisor);
            bcm2835_pwm_set_mode((int)PWMChannel, (int)PWMMarkspace, 0);
            bcm2835_pwm_set_range((int)PWMChannel, PWMRange);
        }
        ...
        public override GPIODirection PinDirection
        {
            ...
            set
            {
                if (PinDirection != (base.PinDirection = value)) // Left to right eval ensures base class gets to check for disposed object access
                {
                    ...
                    bcm2835_gpio_fsel(_pin, (uint)value);
        ...
        public override void SetPWMClock(uint PWMDivisor)
        {
            bcm2835_pwm_set_clock(PWMDivisor);

        }
        public override void SetPWMMode(int markspace, int enabled)
        {
            bcm2835_pwm_set_mode((int)base.Channel, markspace, enabled);
        }
        public override void SetPWMRange(int range)
        {
            bcm2835_pwm_set_range((int)base.Channel, range);
        }

        public override void SetPWMData(uint range)
        {
            bcm2835_pwm_set_data((uint)base.Channel, range);
        }
        ...
        static extern void bcm2835_gpio_fsel(GPIOPins pin, uint mode_out);
        ...
        static extern void bcm2835_pwm_set_clock(uint divisor);
        ...
        static extern void bcm2835_pwm_set_data(uint channel, uint data);
