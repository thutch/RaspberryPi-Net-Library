...
namespace RaspberryPi.GPIO.I2C
{
    ...
    {
        static public String DevR1 = "/dev/i2c-0"; // if Rev1
        static public String DevR2 = "/dev/i2c-1"; // if Rev2
        private int debug = 0;
        ...
        {
            int res = I2CNativeLib.WriteBytes(busHandle, address, bytes, bytes.Length, debug);
            if (res == -1)
        ...
        /// <param name="command"></param>
        /// <param name="data"></param>
        public void WriteCommand(int address, byte command, byte[] bytes)
        {
        ...
        public byte[] ReadBytes(int address, int count)
        {
            ...
            int res = I2CNativeLib.ReadBytes(busHandle, address, buf, buf.Length, debug);
            if (res == -1)
                throw new IOException(String.Format("Error accessing address '{0}': {1}", address, UnixMarshal.GetErrorDescription(Stdlib.GetLastError())));
            if (res <= 0)
